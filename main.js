...
*Hold on it to copy the ID.*`;
    ...
    bot.telegram.getMe().then((getme) => {
        console.table({
            ...
            "Author": "https://t.me/MOTU_PATALU_HINDU_HAI"
        })
    })
...
async function XeonBotIncStart() {
  // ✅ Fetch latest version (auto update-safe)
  const { version, isLatest } = await fetchLatestBaileysVersion();

  // ✅ Multi-file auth ensures persistent session
  ...
  // ✅ Initialize Baileys socket properly
  ...
  // ✅ Save credentials automatically on every update
  ...
  // Export for other functions (like grupkill)
