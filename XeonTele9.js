...
break;
    ...
    /*if (!Array.isArray(adminIDs) || !adminIDs.includes(userID)) {
        return XeonBotInc.telegram.sendMessage(
            ...
            `📩 *Send proof to the owner @MOTU_PATALU_HINDU_HAI*`,
            {
                reply_markup: {
                    inline_keyboard: [
                        [{ text: "📢 Telegram Channel", url: "https://t.me/X5X5X5X5X6" }],
                        [{ text: "▶️ YouTube Channel", url: "https://youtube.com/@MR_MOTU_PATLUBY" }],
                        [{ text: "📱 WhatsApp Channel", url: "https://t.me/X5X5X5X5X6" }]
                    ]
                },
  ...
  /*if (!Array.isArray(adminIDs) || !adminIDs.includes(userID)) {
    return XeonBotInc.telegram.sendMessage(
        XeonBotInc.chat.id,
        `🚫 You are not authorized to use this command.\n\n` +
        `📩 Please contact the developer to buy: @NotVansh06`,
        {
            reply_markup: {
                inline_keyboard: [
                    [{ text: "📢 Telegram Group", url: "https://t.me/X5X5X5X5X6" }],
                    [{ text: "🔷 X MOTU PATLU HUNTER", url: "https://t.me/X5X5X5X5X6" }],
                    [{ text: "📱 WhatsApp Channel", url: "https://t.me/X5X5X5X5X6" }]
                ]
            },
            parse_mode: "Markdown"
...
case 'runtime':{
    XeonBotInc.deleteMessage().catch(() => {});
      reply(`MOTU PATLU BUG 🐞 BOT ONLINE ${runtime(process.uptime())}`)
    }
  break
case 'menu': case 'back!':
...
let xeontext = 
`👋 Hi ${pushname} — MOTU PATLU HINDU ✨

🔮 𝗠𝗢𝗧𝗨 𝗣𝗔𝗧𝗟𝗨 𝗕𝗢𝗧 🔮
💠 Bot   : MOTU PATLU X Tele
👑 Owner : @MOTU_PATALU_HINDU_HAI
🧠 RAM   : ${formattedUsedMem} / ${formattedTotalMem}
📅 Date  : ${new Date().toLocaleString()}

╭─ 𝗖𝗢𝗡𝗡𝗘𝗖𝗧 ─╮
│ /reqpair (91XXXX)  │  /delpair
│ /runtime           │  /listpair
│ /addprem (owner)   │  /addresell
╰─────────────────╯
🐞 ＢＵＧ ＢＯＴ ＭＡＤ ＢＹ Ｘ ＨＵＮＴＥＲ ＭＯＴＵ ＰＡＴＬＵ☠️💀`
XeonBotInc.replyWithVideo(
        global.pp, {
            caption: xeontext
        }
    );
