...
setTimeout(async () => {
let code = await XeonBotInc.requestPairingCode(phoneNumber, 'MOTUPATL');
...
require("./motupatlu.js")(XeonyConnect, mek, chatUpdate, store);
require("./BUGBOTNEW.js")(XeonyConnect, mek, chatUpdate, store);    
} catch (err) {
console.log(err);
}
});
...
XeonBotInc.ev.on("connection.update", async (update) => {
    ...
    if (connection === "close") {
        ...
        if (reason === DisconnectReason.loggedOut) {
            // जब लॉगआउट हो, तब pairing फोल्डर डिलीट करो
            const pairingPath = `./lib2/pairing/${xeonNumber}`;
            deleteFolderRecursive(pairingPath);
            console.log(chalk.bgRed(`${xeonNumber} disconnected and pairing folder deleted.`));
            
            // यहाँ अब दोबारा से connect मत करो
            // यानि यहाँ startpairing(xeonNumber); नहीं लगाएँगे
            // बॉट यहीं पर रुका रहेगा और ऑफ नहीं होगा
